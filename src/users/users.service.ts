...
interface ListOpts { q?: string; page?: number; pageSize?: number; }
...
export class UsersService {
  ...
  async create(reqUser: any, dto: any) {
    ...
    if (dto.roleId !== undefined && dto.roleId !== null) {
      ...
      const role = await this.prisma.role.findUnique({ where: { name: String(dto.role) }});
      if (role) roleIdToUse = role.id;
    }

    if (creatorRole === 'TENANT_ADMIN') {
      if (roleIdToUse) {
        const roleCheck = await this.prisma.role.findUnique({ where: { id: roleIdToUse }});
        if (roleCheck?.name === 'SYSTEM_ADMIN') {
          throw new ForbiddenException('TENANT_ADMIN cannot create SYSTEM_ADMIN users');
        }
      }
    ...
    const passwordHash = dto.password ? await this.hashPassword(dto.password) : '';
  ...
  async update(user: any, id: string | number, dto: any) {
    const target = await this.prisma.user.findUnique({ where: { id: BigInt(String(id)) } });
    ...
    if ((user?.role ?? '').toString() === 'TENANT_ADMIN') {
      ...
      if (dto.roleId) {
        const role = await this.prisma.role.findUnique({ where: { id: Number(dto.roleId) }});
        if (role?.name === 'SYSTEM_ADMIN') throw new ForbiddenException('Cannot set SYSTEM_ADMIN');
      }
    ...
    if (dto.password) updateData.passwordHash = await this.hashPassword(dto.password);
    if (dto.roleId !== undefined) updateData.roleId = dto.roleId;
    if (dto.tenantId !== undefined) updateData.tenantId = dto.tenantId ? BigInt(String(dto.tenantId)) : null;
